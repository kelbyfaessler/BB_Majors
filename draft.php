...
<title>Draft Team</title>
	...
	function UpdateSalaries()
	{
		...
		$("#player_list tr").not("thead tr").each(function(){
			...
			{
				$(".AddGolfer", $(this)).attr("disabled","disabled");
			}
			else
			{
				$(".AddGolfer", $(this)).removeAttr("disabled");
			}
		});
	}
	$(document).ready(function(){
		$("#player_list").DataTable({
			...
			"info":false,
			"order":[[1,"desc"]],
			"dom": '<"pull-left"f>rt<"bottom"lp><"clear">'
		});
		UpdateSalaries();
...
<div style='width: 800px;margin:auto; margin-top:75px;top: 0;right: 0;bottom: 0;left: 0;height:auto;background-color: #ccc;border-radius: 8px;overflow:hidden;'>

<div><a style='color:#00E' href="MajorsBBHome.php">&#60&#60Return Home</a></div>
		...
		echo "<div style='text-align:center'><h1 style='text-decoration: underline;color:#000000'>Draft team for ".$tourn."</h1></div>";
		echo "</br>";
		echo "<div id='players' style='padding-left:20px;float:left;width:40%'>";
        echo "<table id='player_list'>
			<thead>
				<tr>
					<th>Player</th>
					<th>Salary</th>
    ...
    <div id="roster" style="width:40%;float:right;">
	<div><button id="submitChanges">Submit Changes</button></div>
        <?php
		...
		for($i=1; $i < 5; $i++)
		{
			...
			echo "
					...
					<div class='roster_round_table'><table player_id='".$player."' scorecard_id='".$scorecard_id."' locked=".$isLocked.">
							<caption>Round ".$i."</caption>
							<thead><tr>";
								if(!$isLocked)
									echo "<th><button class='edit_roster_round' id='edit_round_".$i."'>Edit</button></th><th colspan=3><div style='text-align:right'><button class='copy_down' id='copy_down_round_".$i."'>Copy Down</button></div></th>";
								else
									echo "<th></th>";
		...
		?>
		<div><button id="clearEntry">Clear</button></div>
    </div>
